...
int main(int argc, char *argv[]) {
  ...
  case 1:
    print_help();
    break;
  /* 3RD ARGUMENT IS (MOSTLY) OPTIONAL SO THERE IS NO (BIG) DIFFERENCE BETWEEN 2
   * AND 3 ARGUMENTS*/
  case 2:
  case 3:
      /* -------------------- ERROR CHECKING -------------------- */
    ...
    char home_dir[38]; // 38 because of the fact that username can be 32
                       // characters long + 6 characters for the /home/
    first_option_letter = argv[1][0];
    strcpy(home_dir, getenv("HOME"));
    int error_type = 0;

    error_type =
        inline_arguments_check(argc, argv, first_option_letter, home_dir);
    switch (error_type) {
    case 1:
      printf("There is no such option! Run beli with 'help' flag for more "
             "information.\n");
      return 1;
      break;
    ...
    case 3:
      printf("Invalid list name length! List names can be up to 15 characters "
             "long.\n");
      return 1;
      break;
    ...
    case 5:
      printf("Invalid list name! List names can only be letters, numbers and "
             "underscores.\n");
      return 1;
      break;
    ...
    case 8:
      printf("Cannot add items to a non-existing list.\n");
      return 1;
      break;
    default:
    ...
    /* DECLARED VARIABLES TO GET 'HOME' VARIABLE AND SUBSEQUENTLY FULL PATH TO
     * CONFIG FILE */
    char list_name[20]; // 20 because 15 chars maximum for name and + 5 for
                        // .beli extension
    ...
    break;
  default:
    printf("Too many arguments! Run beli with 'help' flag for more "
           "information.\n");
    return 1;
    break;
  }
